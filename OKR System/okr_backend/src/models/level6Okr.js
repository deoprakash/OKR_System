...
const Level6OKRSchema = new mongoose.Schema(
  {
    level5OkrCode: { type: Number, required: true, index: true },
    level6OkrCode: { type: Number, unique: true, index: true },
    empLevel: { type: Number, default: 6 },
...
// Auto-generate a running serial number for `level6OkrCode`
Level6OKRSchema.pre("save", async function (next) {
  if (!this.isNew) return next();
  if (this.level6OkrCode) return next();
  ...
  try {
    ...
    this.level6OkrCode = counter.seq;
    next();
  } catch (err) {
    next(err);
  }
...
const Level6OKR = mongoose.model("Level6OKR", Level6OKRSchema);

export default Level6OKR;
