...
const OKRWorkspaceLevel2 = () => {
    ...
    async function load() {
      try {
        ...
        if (fields.employeeCode) {
          ...
          if (match) setFields(f => ({ ...f, userId: match.userId || (match._id ? String(match._id) : '') }));
        }
  ...
  useEffect(() => {
    ...
    if (emp && emp.userId && emp.userId !== fields.level1userId) {
      setFields(f => ({ ...f, level1userId: emp.userId }));
    }
  ...
  const handleSelectLevel1Employee = (e) => {
    ...
    setFields(f => ({ ...f, level1EmployeeCode: code, level1EmployeeName: emp ? emp.empName : '', level1userId: emp ? emp.userId : '', level1OKRDescription: '' }));
        ...
        <form>
          ...
          <Box>
            ...
            <FormRow>
              ...
              <div className="w-62">
                <label className="font-semibold block mb-1">Employee Code</label>
                <input value={fields.level1userId} readOnly className="border px-2 py-1 w-full bg-gray-100" />
              </div>
