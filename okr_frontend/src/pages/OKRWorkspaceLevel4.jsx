...
import { listEmployees, listLevel3OKRs, listLevel4OKRs, getLevel3OKR, createLevel4OKR, updateLevel4OKR } from '../lib/api';
  ...
  const handleSelectOKRCode = async (e) => {
    ...
    if (val === 'NEW') {
      const newFields = { ...fields, 
        ...
        okrDescription: '', 
        keyResults: Array(5).fill(''), 
        quarters: [ { percent: '', comment: '' }, { percent: '', comment: '' }, { percent: '', comment: '' }, { percent: '', comment: '' } ], okrDate: getLocalDateString() };
      setFields(newFields);
      setIsDirty(false);
      pristineRef.current = JSON.stringify(newFields);
      return;
    ...
    let parent = null;
      ...
      try {
        ...
        const parentRes = await getLevel3OKR(
          okr.level3OkrCode
        );

        parent = parentRes.data;

        const allRes = await listLevel3OKRs();
        ...
        const descriptions = (allRes.data || [])
          ...
          .map(x => ({
            level3OkrCode: x.level3OkrCode,
            okrDesc: x.okrDesc,
          }));

        setLevel3OKRDescriptions(descriptions);
    ...
    const newFields = {
      ...
      level3OkrCode:
        parent?.level3OkrCode || "",

      level3OKRDescription:
        parent?.okrDesc || "",

      okrCode: okr.level4OkrCode,
  ...
  const handleUpdateOKR = async () => {
    ...
    try {
      const payload = {
        ...
        level3OkrCode: fields.level3OkrCode
          ? Number(fields.level3OkrCode)
          : undefined,
                ...
                {level4All
                  ...
                  .filter(
                    (v, i, a) =>
                      a.findIndex(
                        (t) =>
                          String(t.level4OkrCode) === String(v.level4OkrCode),
                      ) === i,
                  )
                  .map((o) => (
                    <option key={o.level4OkrCode} value={o.level4OkrCode}>
                      {o.okrDesc?.slice(0, 50) || String(o.level4OkrCode)}
                    </option>
                  ))}
              </select>
            </div>
          ...
          <Box>
            ...
            <FormRow>
              <div className="w-62">
                ...
                <select
                  ref={firstInputRef}
                  value={fields.level3EmployeeCode}
                  onChange={handleSelectLevel3Employee}
              ...
              <div className="w-full">
                ...
                <select
                  value={fields.level3OKRDescription}
                  onChange={(e) =>
                    setFields((f) => ({
                      ...f,
                      level3OKRDescription: e.target.value,
                      level3OkrCode:
                        e.target.value &&
                        (() => {
                          try {
