...
const OKRWorkspaceLevel4 = () => {
  ...
  const handleUpdateOKR = async () => {
    ...
    try {
      ...
      alert('Invalid OKR Date'); return;
    }
    if (!fields.level3OkrCode) { alert('Please select a Level-3 OKR to link before saving.'); return; }
    try {
      const payload = {
      ...
      if (fields.okrCode === 'NEW' || fields.okrCode === '' || fields.okrCode == null) {
        ...
        alert('Created OKR with code: ' + (created.level4OkrCode || created._id));
            ...
            <div className="flex items-center gap-1 ml-4">
              ...
              <select value={fields.okrCode} onChange={handleSelectOKRCode} className="border px-2 py-1 w-48">
                ...
                {level4All.filter(o => Number(o.empCode) === Number(fields.employeeCode)).map(o => (
                  <option key={o.level4OkrCode} value={o.level4OkrCode}>{o.level4OkrCode} - {o.okrDesc?.slice(0,50)}</option>
                ))}
            ...
            <FormRow>
              <div className="w-32">
                <label className="font-semibold block mb-1">Employee Code</label>
                  <select ref={firstInputRef} value={fields.level3EmployeeCode} onChange={handleSelectLevel3Employee} className="border px-2 py-1">
                  <option value="">-- Select --</option>
