...
const OKRWorkspaceLevel6 = () => {
  ...
  const handleUpdateOKR = async () => {
    ...
    if (!fields.level5OkrCode) { alert('Please select a Level-5 OKR to link before saving.'); return; }
    ...
    try {
      ...
      if (fields.okrCode === 'NEW' || fields.okrCode === '' || fields.okrCode == null) {
        ...
        alert('Created OKR with code: ' + (created.level6OkrCode || created._id));
            ...
            <div className="flex items-center gap-1 ml-4">
              ...
              <select value={fields.okrCode} onChange={handleSelectOKRCode} className="border px-2 py-1 w-48">
                ...
                {level6All.filter(o => Number(o.empCode) === Number(fields.employeeCode)).map(o => (
                  <option key={o.level6OkrCode} value={o.level6OkrCode}>{o.level6OkrCode} - {o.okrDesc?.slice(0,50)}</option>
                ))}
            ...
            <FormRow>
              <div className="w-32">
                <label className="font-semibold block mb-1">Employee Code</label>
                  <select ref={firstInputRef} value={fields.level5EmployeeCode} onChange={handleSelectLevel5Employee} className="border px-2 py-1">
                  <option value="">-- Select --</option>
